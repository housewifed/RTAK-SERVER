...
wget https://github.com/housewifed/RTAK-SERVER/releases/download/v1.0.5/rtak-server-1.0.5-linux-amd64.run
chmod +x rtak-server-1.0.5-linux-amd64.run
sudo ./rtak-server-1.0.5-linux-amd64.run
```
...
```bash
sudo ./rtak-server-1.0.5-linux-amd64.run --yes --host rtak.example.com
```
...
```bash
sudo ./rtak-server-1.1.0-linux-amd64.run --yes
```
